...
public class ByteHashFunctionP {
	...
	public int hashBytes(byte[] data, int start, int end, int pNum) {
		int value = 0;
		if (pNum > 2) {
			ByteHashFuctionParaTask left = new ByteHashFuctionParaTask(0, end / 2, data, pNum / 2, hashProp);
			ByteHashFuctionParaTask right = new ByteHashFuctionParaTask(end / 2, data.length, data, pNum / 2, hashProp);
			left.fork();
			right.fork();
